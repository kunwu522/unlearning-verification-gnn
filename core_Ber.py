...
class Smooth_Ber(object):
    ...
    def __init__(self, base_classifier: torch.nn.Module, num_classes: int, dim: int, prob: float, adj: torch.tensor, fea: torch.tensor, device: torch.device):
        """
        ...
        self.fea = fea.to(device)
        self.num_nodes = len(self.fea)
        self.device = device
        # self.cuda = cuda
        self.m = Bernoulli(torch.tensor([self.prob], device=self.device))
        # if self.cuda:
    ...
    def _sample_noise_ber(self, idx: int, num: int, batch_size: int):
        ...
        adj = self.adj.to_dense().int().clone().detach().to(self.device)
        ...
        with torch.no_grad():
            ...
            # for _ in tqdm(range(num), desc='sample noise bar'):
            for _ in range(num): 
                mask = self.m.sample(adj[idx].shape).squeeze(-1).int()
                rand_inputs = torch.randint_like(adj[idx], low=0, high=2, device=self.device).squeeze().int()
                ...
                adj_noise_norm = normalize(torch.eye(self.num_nodes, device=self.device) + adj_noise)
