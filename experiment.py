    ...
    for length in range(len(triggers) - 1, 0, -1):
        for _edges in combinations(triggers, length):
            ...
            if unlearn_method == 'retrain':
                ...
                neg_res = neg_model.unlearn(_edges)
                sub_adv_pred = neg_model.predict([target_node], use_retrained=True)[0]


            if sub_adv_pred != adv_pred:
                incompleteness = True
                break
        if incompleteness:
            break
...
def certify_fragile_with_perturbations(inputs, args, verifier, data):
    ...
    while len(ps) < args.num_perts:
        ...
        if res['fragile']:
            p = tuple(res['perturbations'][0])
            if target_node not in p:
                print('  ==> certifing', target_node, 'error:', p, 'not in target node')
...
def check_1pf(args, original, target, data, device, v, candidates, c, num_layer=2, unlearn_method='retrain', verbose=False):
    if unlearn_method == 'gnndelete':
        weights, deletion = copy.deepcopy(target.parameters(use_retrained=True))
        weights = [p.cpu().detach().numpy() for p in weights]
        deletion = [p.cpu().detach().numpy() for p in deletion]
        # weights, deletion = copy.deepcopy([p.cpu().detach().numpy() for p in target.parameters(use_retrained=True)])
        verifier = CertifiedFragilenessLazy(args, weights, data, num_layer=num_layer, verbose=False, deletion=deletion)
    else:
        weights = copy.deepcopy([p.cpu().detach().numpy() for p in target.parameters(use_retrained=True)])
        verifier = CertifiedFragilenessLazy(args, weights, data, num_layer=num_layer, verbose=False)

    # _weights = copy.deepcopy([p.cpu().detach().numpy() for p in original.parameters()])
    # original_verifier = CertifiedFragilenessLazy(args, _weights, data)
    
    for candidate in candidates:
        if isinstance(target, unlearn.Unlearn):
            pred, post = target.predict(target_nodes=[v], return_posterior=True, use_retrained=True)
        else:
            pred, post = target.predict(data, device, target_nodes=[v], return_posterior=True)
        _c = _second_best_labels(post)[0].item()
        # v, perturbation, c = certify_fragile_with_perturbations((v, pred, c), args, verifier, data)
        if c != pred[0]:
            res = verifier.certify(v, pred[0], c, [candidate], solver=args.solver, mask=[])
        else:
            _candidates = data.train_set.nodes[data.train_set.y == _c].tolist()
            _candidates = list(set(_candidates) - set(data.adj_list[target]))
            candidate = random.sample(_candidates, 1)
            res = verifier.certify(v, pred[0], _c, candidate, solver=args.solver, mask=[])
        # res = verifier.certify(v, pred, c if c != pred else _c, [candidate], solver=args.solver, mask=[])
        # _res = original_verifier.certify(v, pred, c, [candidate], solver=args.solver, mask=[])
        if res['fragile']:
            return True
    ...
    while len(v_star) < args.num_target_nodes:
        ...
        if surrogate_data is not None:
            ...
            if isinstance(target, unlearn.Unlearn):
                ...
                pred, post = target.predict(data, device, target_nodes=[v], return_posterior=True)
        surro_pred = surrogate.predict(data, device, target_nodes=[v])
        if pred != surro_pred:
            continue
...
def batch_detection(args):
    ...
    for t in range(args.num_trials):
        ...
        detection_result = {
            ...
            'js': defaultdict(list),
            'pd': defaultdict(list),
            'ggd': defaultdict(list),
        }
        used_token_nodes = []
        for _ in range(_num_behaviors):
            v_star, e_star, c_star = find_1pf(args, target_model, surrogate, data, device, used_token_nodes=used_token_nodes, verbose=True)
            p_star = target_model.predict(data, device, target_nodes=v_star)
            ...
            # mapping_idx = subdata.subgraph_by_edges(len(perturbations) * 100, [v for e in perturbations for v in e])
            # v_star = [v for v in v_star if v in mapping_idx]
            # e_star = [[[mapping_idx[u], mapping_idx[v]] for u, v in e] for e in e_star]

            detection_methods = ['lp', 'od', 'ggd']
            # for detector_type in ['lp', 'od', 'js', 'ggd']:
            for detector_type in detection_methods:
                if detector_type == 'lp':
                    detector = LinkPredDetector(data, v_star, e_star, device)
                elif detector_type == 'od':
                    detector = OutlierDetector(data, v_star, e_star)
                ...
                if detector_type == 'pd':
                    if len(detection) != len(v_star):
                        print(detection)
                    _remaining_nodes = np.array(v_star)[~detection.astype(bool)].tolist()
                    _detection_count[detector_type]['num nodes'].append(len(v_star))
                    _detection_count[detector_type]['num detected nodes'].append(len(v_star) - len(_remaining_nodes))
                    _detection_count[detector_type]['num remain nodes'].append(len(_remaining_nodes))
                    if len(_remaining_nodes) == 0:
                        _detection_count[detector_type]['detected all'].append(1)
                    else:
                        _detection_count[detector_type]['detected all'].append(0)
                else:
                    if len(detection) != len(perturbations):
                        print(detection)
                    _remaining_edges = np.array(perturbations)[~detection.astype(bool)].tolist()
                    _detection_count[detector_type]['num edges'].append(len(perturbations))
                    _detection_count[detector_type]['num detected edges'].append(len(perturbations) - len(_remaining_edges))
                    _detection_count[detector_type]['num remain edges'].append(len(_remaining_edges))
                    if len(_remaining_edges) == 0:
                        _detection_count[detector_type]['detected all'].append(1)
                        # continue
                    else:
                        _detection_count[detector_type]['detected all'].append(0)
            ...
            for detector_type in detection_methods:
                detection_result[detector_type]['num edges'].extend(_detection_count[detector_type]['num edges'])
                detection_result[detector_type]['num detected edges'].extend(_detection_count[detector_type]['num detected edges'])
                detection_result[detector_type]['num remain edges'].extend(_detection_count[detector_type]['num remain edges'])
                detection_result[detector_type]['detected all'].extend(_detection_count[detector_type]['detected all'])
            # for detector_type in ['pd']:
            #     detection_result[detector_type]['num nodes'].extend(_detection_count[detector_type]['num nodes'])
            #     detection_result[detector_type]['num detected nodes'].extend(_detection_count[detector_type]['num detected nodes'])
            #     detection_result[detector_type]['num remain nodes'].extend(_detection_count[detector_type]['num remain nodes'])
            #     detection_result[detector_type]['detected all'].extend(_detection_count[detector_type]['detected all'])
        ...
        result['TPR'].append(tp / _num_behaviors)
        result['TNR'].append(tn / _num_behaviors)
        for d_type in detection_methods:
            # result[f'TPR {d_type}'].append(sum(detection_result[d_type]['tp']) / _num_behaviors)
            # result[f'TPR {d_type} 2'].append(sum(detection_result[d_type]['tp']) / (_num_behaviors - sum(detection_result[d_type]['detected all'])))
            ...
            result[f'detection ratio {d_type}'].append(sum(detection_result[d_type]['num detected edges']) / sum(detection_result[d_type]['num edges']))
        # for d_type in ['pd']:
        #     result[f'detection ratio {d_type}'].append(sum(detection_result[d_type]['num detected nodes']) / sum(detection_result[d_type]['num nodes']))
    ...
    df.to_csv(os.path.join('result', f'batch_detection_unlearning_{args.dataset}_{ts}.csv'))

    print('-' * 80)
    print('The result is saved to', f'batch_detection_unlearning_{args.dataset}_{ts}.csv')
    print('-' * 80)
            ...
            for p in [0.2]:
                ...
                for idx in tqdm(range(len(_candidates)), desc=f'At trial {t}, ours verifying {p}%'):
                    ...
                    if len(trial_perts) != args.num_perts:
                        logger.warning('The number of perturbations is less than 10, but %d', len(trial_perts))
...
def verification_with_detection(args):
    ...
    for t in range(args.num_trials):
        ...
        for _ in tqdm(range(_num_behaviors), desc=f'verify vs detect'):
            v_star, e_star, c_star = find_1pf(args, target_model, surrogate, data, device, used_token_nodes=used_token_nodes)
            p_star = target_model.predict(data, device, target_nodes=v_star)
            used_token_nodes.extend(v_star)
            ...
            for v, e, p, c in zip(v_star, e_star, p_star, c_star):
                ...
                # neg_pred = _unlearn_train_predict(args, adv_data, e, [v], device)[0]
                # if neg_pred != adv_pred:
                #     _tn_count += 1
                ...
                for detector_type in ['lp', 'od', 'js']:
                    ...
                    detection = detector.detect_all()
                    if len(detection) != len(e):
                        print(detection)
                    _remaining_edges = np.array(e)[~detection.astype(bool)].tolist()
                    _detection_count[detector_type]['num edges'].append(len(e))
                    _detection_count[detector_type]['num detected edges'].append(len(e) - len(_remaining_edges))
                    _detection_count[detector_type]['num remain edges'].append(len(_remaining_edges))
                    if len(_remaining_edges) == 0:
                        _detection_count[detector_type]['detected all'].append(1)
                        continue
                    ...
                    _detection_count[detector_type]['detected all'].append(0)
                    # retain_edges = tuple(random.sample(_remaining_edges, 1)[0])
                    # challenge_edges = copy.deepcopy(e)
                    # challenge_edges.remove(retain_edges)

                    # pos_unlearn_data = copy.deepcopy(adv_data)
                    # pos_unlearn_data.remove_edges(challenge_edges)
                    # pos = GNN(args, pos_unlearn_data.num_features, pos_unlearn_data.num_classes, surrogate=False, fix_weight=False)
                    # pos.train(pos_unlearn_data, device)
                    # pos_pred = pos.predict(pos_unlearn_data, device, target_nodes=[v])[0]
                    # if pos_pred == adv_pred:
                    #     _detection_count[detector_type]['tp'].append(1)
            ...
            if _tn_count == args.num_target_nodes:
                tn += 1
            for detector_type in ['lp', 'od', 'js']:
                # if sum(_detection_count[detector_type]['tp']) > 0:
                #     # tp_detection[detector_type] += 1
                #     detection_result[detector_type]['tp'].append(1)
                detection_result[detector_type]['num edges'].extend(_detection_count[detector_type]['num edges'])
                detection_result[detector_type]['num detected edges'].extend(_detection_count[detector_type]['num detected edges'])
                detection_result[detector_type]['num remain edges'].extend(_detection_count[detector_type]['num remain edges'])
                detection_result[detector_type]['detected all'].extend(_detection_count[detector_type]['detected all'])
    ...
    df.to_csv(os.path.join('result', f'detection_verify_unlearning_{args.dataset}_{ts}.csv'))

    print('-' * 80)
    print('The result is saved to', f'detection_verify_unlearning_{args.dataset}_{ts}.csv')
    print('-' * 80)
...
def find_perturbations(args):
    ...
    result = defaultdict(list)
    for t in range(args.num_trials):
        logger.info('Start verifing unlearning at trial %d ...', t)
        if not is_fixed_seed:
            args.seed = random.randint(0, 1e+5)
            torch.manual_seed(args.seed)
            torch.cuda.manual_seed_all(args.seed)
        ...
        logger.info('  ==> Loading the target model ...')
        t0 = time.time()
        target_model = _get_target_model(args, data, device)
        ...
        for method in args.methods:
            logger.info('  ==> Sampling node tokens ...')
            if args.node_sampler == 'boundary':
    ...
    for t in range(args.num_trials):
        ...
        for method in args.methods:
            ...
            if method in ['nettack', 'minmax', 'ig', 'pgd', 'gf']:
                ...
                # logger.info('  ==> Loading the surrogate model ...') 
                surrogate = GNN(args, surrogate_data.num_features, surrogate_data.num_classes, surrogate=False, fix_weight=False)
                surrogate.train(data, device)
            ...
            for num_node_tokens in [args.num_target_nodes]:
                ...
                for p in args.percentages:
                    ...
                    for idx in tqdm(range(len(_candidates)), desc=f'At trial {t}, {method} verifying {p}%'):
                        ...
                        trial_perts = _perturbations[idx]
                        if len(trial_perts) != args.num_perts:
                            logger.warning(f'The number of perturbations is less than {args.num_perts}, but %d', len(trial_perts))
                        # trial_pred = _predictions[idx]
...
def _verify_unlearning(args):
    ...
    for t in range(args.num_trials):
        ...
        surrogate = GNN(args, surrogate_data.num_features, surrogate_data.num_classes, surrogate=False, fix_weight=False)
        surrogate.train(data, device)
        logger.info('  ==> start to run CVX searching ...')
        verifier = CertifiedFragileness(args, surrogate, data, device, verbose=False)
        v_star, e_star = [], []
        c_star, p_star = [], []
        test_nodes = data.test_set.nodes.tolist()
        ...
        for num_node_tokens in [args.num_target_nodes]:
            ...
            for p in args.percentages:
                ...
                for idx in tqdm(range(len(_candidates)), desc=f'At trial {t}, {method} verifying {p}%'):
                    ...
                    trial_perts = _perturbations[idx]
                    if len(trial_perts) != args.num_perts:
                        logger.warning('The number of perturbations is less than 5, but %d', len(trial_perts))
                    # trial_pred = _predictions[idx]
...
def efficiency_surrogate(args):
    ...
    for t in range(args.num_trials):
        ...
        t0 = time.time()
        target_model.train(data, device)
        target_train_time = time.time() - t0
        target_train_preds = target_model.predict(data, device, target_nodes=data.train_set.nodes.tolist())
        result['target train time'].append(target_train_time)

        surrogate_data = copy.deepcopy(data)
        surrogate_data.train_set.y = torch.from_numpy(target_train_preds)
        surrogate = GNN(args, surrogate_data.num_features, surrogate_data.num_classes, surrogate=True, fix_weight=False)
        t0 = time.time()
        surrogate.train(surrogate_data, device)
        surrogate_train_time = time.time() - t0
        result['surrogate train time'].append(surrogate_train_time)
    ...
    df.to_csv(os.path.join('result', f'efficiency_training_time_{args.dataset}_{ts}.csv'))
    print('Efficiency training time is done, the result is saved to', f'efficiency_training_time_{args.dataset}_{ts}.csv')
...
def rebuttal(args):
    """ Experiments for rebuttal
        1. The accuracy of the different surrogate models and their loss comparing with the target model
        2. Measure the perparation time for the different surrogate models
    """
    device = utils.get_device(args)
    ...
    for t in range(args.num_trials):
        ...
        target_res = target_model.evaluate(data, device)
        logger.info('-' * 80)
        logger.info(f'target results at trial {t}:')
        logger.info(f'==> accuracy: {target_res["accuracy"]:.4f}')
        logger.info(f'==> f1: {target_res["f1"]:.4f}')
        logger.info(f'==> loss: {target_res["loss"]:.4f}')
        logger.info(f'==> running time: {time.time() - t0 + 0.9:.4f}')
        ...
        args.num_target_nodes = 1
        v_star, e_star, c_star = find_1pf(
            args, target_model, surrogate, data, device, num_layer=2, verbose=False,
        ...
        for v, e, p, c in zip(v_star, e_star, p_star, c_star):
            ...
            adv = GNN(args, adv_data.num_features, adv_data.num_classes, surrogate=False, fix_weight=False)
            adv.train(adv_data, device)
            res = adv.evaluate(adv_data, device)
            running_time = time.time() - t0 + 0.9
            logger.info('-' * 80)
            logger.info(f'running time for the adversarial training: {running_time:.4f}')

            for unlearn_method in ['gnndelete']:
                t0 = time.time()
                if unlearn_method == 'eraser':
                    adv = unlearn.GraphEraser(
                        args.seed, data.x, data.adjacency_matrix().to_dense(), adv_data.y, 
                        config, device, model_type='gcn', epochs=args.epochs, patience=args.patience
                    )
                elif unlearn_method == 'gnndelete':
                    adv = unlearn.GNNDelete(
                        args.seed, data, data.x, data.adjacency_matrix().to_dense(), adv_data.y,
                        config, device, model_type='gcn', epochs=args.epochs, verbose=args.verbose
                    )
                adv_res = adv.train()
                preparation_time = time.time() - t0
                logger.info('-' * 80)
                logger.info(f'unlearn results at trial {t} with {unlearn_method}:')
                logger.info(f'==> accuracy: {adv_res["accuracy"]:.4f}')
                logger.info(f'==> f1: {adv_res["weighted avg"]["f1-score"]:.4f}')
                logger.info(f'==> loss: {adv_res["loss"]:.4f}')
                logger.info(f'==> preparation time: {preparation_time:.4f}')
                # res = target_model.train()

                neg_model = copy.deepcopy(adv)
                t0 = time.time()
                # logger.info(f'e: {e}, type: {type(e)}')
                neg_res = neg_model.unlearn(e)
                running_time = time.time() - t0
                logger.info('-' * 80)
                logger.info(f'==> accuracy: {neg_res["accuracy"]:.4f}')
                logger.info(f'==> f1: {neg_res["weighted avg"]["f1-score"]:.4f}')
                logger.info(f'==> loss: {neg_res["loss"]:.4f}')
                logger.info(f'==> running time: {running_time:.4f}')
        ...
        #     if surrogate_type == 'simplified':
        #         surrogate = GNN(args, data.num_features, data.num_classes, surrogate=True, fix_weight=False, bias=False)
        #         surrogate.train(data, device)
        ...
        #     res = surrogate.evaluate(data, device)
        ...
        #     preparation_time = time.time() - t0
            ...
            # logger.info('-' * 80)
            # logger.info(f'surrogate results at trial {t} with {surrogate_type}:')
            # logger.info(f'==> accuracy: {res["accuracy"]:.4f}')
            # logger.info(f'==> f1: {res["f1"]:.4f}')
            # logger.info(f'==> loss: {res["loss"]:.4f}')
        #     logger.info(f'==> preparation time: {preparation_time:.4f}')

        # for target in ['gat', 'sage', 'gin']:
        # for target in ['gin']:
        #     args.target = target
        #     _target_model = GNN(args, data.num_features, data.num_classes, surrogate=False, fix_weight=False)
        #     _target_model.train(data, device)
        #     _target_res = _target_model.evaluate(data, device)
        #     logger.info('-' * 80)
        #     logger.info(f'target results at trial {t} withG {target}:')
        #     logger.info(f'==> accuracy: {_target_res["accuracy"]:.4f}')
        #     logger.info(f'==> f1: {_target_res["f1"]:.4f}')
        #     logger.info(f'==> loss: {_target_res["loss"]:.4f}')
        ...
        #     'idx_test': data.test_set.nodes.tolist(),
        # }
        # for unlearn_method in ['eraser', 'gnndelete']:
        #     if  unlearn_method == 'eraser':
        #         unlearn_target_model = unlearn.GraphEraser(
        #             args.seed, data.x, data.adjacency_matrix().to_dense(), data.y, 
        #             config, device, model_type='gcn', epochs=args.epochs, patience=args.patience
        #         )
        #         # res = target_model.train()
        #     elif unlearn_method == 'gnndelete':
        #         unlearn_target_model = unlearn.GNNDelete(
        #             args.seed, data, data.x, data.adjacency_matrix().to_dense(), data.y,
        #             config, device, model_type='gcn', epochs=args.epochs, verbose=args.verbose
        #         )
        #     res = unlearn_target_model.train()
        #     # print(f'  ==> The {unlearn_method} is trained, the result, acc: {res["accuracy"]}, F1: {res["weighted avg"]["f1-score"]} loss: {res["loss"]}')
        #     logger.info('-' * 80)
        #     logger.info(f'unlearn results at trial {t} with {unlearn_method}:')
        #     logger.info(f'==> accuracy: {res["accuracy"]:.4f}')
        #     logger.info(f'==> f1: {res["weighted avg"]["f1-score"]:.4f}')
        #     logger.info(f'==> loss: {res["loss"]:.4f}')
...
def vary_surrogate(args):
    ...
    confident_score_target, confident_score_surrogate = {}, {}
    accuracy_target, accuracy_surrogate = {}, {}
    # for t in range(args.num_trials):
    t = args.trial
    for t in range(args.num_trials):
        args.seed = random.randint(0, 1e+5)
        ...
        confident_score_target[t] = {}
        confident_score_surrogate[t] = {}
        # accuracy_target[t] = {}
        # accuracy_surrogate[t] = {}

        # for surrogate_type in ['simplified', 'standard', 'large', '3-layer', 'target']:
        for surrogate_type in ['simplified', '4-neurons', '8-neurons', 'standard']:
            confident_score_target[t][surrogate_type] = []
            confident_score_surrogate[t][surrogate_type] = []
            ...
            for _ in tqdm(range(_num_behaviors), desc=f'method={surrogate_type}'):
                ...
                p_star, target_post = target_model.predict(data, device, target_nodes=v_star, return_posterior=True)
                confident_score_target[t][surrogate_type].append(utils.confidence(target_post))
                _, surrogate_post = surrogate.predict(data, device, target_nodes=v_star, return_posterior=True)
                confident_score_surrogate[t][surrogate_type].append(utils.confidence(surrogate_post))
            ...
            result['tpr'].append(tp / _num_behaviors)
            result['tnr'].append(tn / _num_behaviors)
            result['confident score target'].append(np.mean(confident_score_target[t][surrogate_type]))
            result['confident score surrogate'].append(np.mean(confident_score_surrogate[t][surrogate_type]))
        trial_df = pd.DataFrame(result)
        trial_df = trial_df[trial_df['trial'] == t]
...
def vary_unlearning_batch(args):
    device = utils.get_device(args)
    ...
    for t in range(args.num_trials):
        ...
        confidence_target[t] = {}
        confidence_surrogate[t] = {}
        # args.num_target_nodes = _calc_m(_alpha, _beta, _p[args.dataset]['ours'], _q[args.dataset]['ours'])
        args.num_target_nodes = 2

        # for unlearn_method in ['retrain', 'eraser', 'ceu', 'gnndelete', 'megu']:
        for unlearn_method in ['megu']:
            confidence_target[t][unlearn_method] = []
            confidence_surrogate[t][unlearn_method] = []
            ...
            if unlearn_method == 'eraser':
                target_model = unlearn.GraphEraser(
                    args.seed, data.x, data.adjacency_matrix().to_dense(), data.y, 
                    config, device, model_type='gcn', epochs=args.epochs, 
                ...
                target_train_preds = target_model.predict(data.train_set.nodes.tolist())
                target_valid_preds = target_model.predict(data.valid_set.nodes.tolist())
            elif unlearn_method == 'megu':
                target_model = unlearn.MEGU(
                    args.seed, data.x, data.adjacency_matrix().to_dense(), data.y,
                    config, device, model_type='gcn', epochs=args.epochs, patience=args.patience,
                    edge_index=data.edge_index, verbose=args.verbose,
                    # alpha1=args.alpha1, alpha2=args.alpha2, kappa=args.kappa,
                )
                res = target_model.train()
                target_train_preds = target_model.predict(data.train_set.nodes.tolist())
                target_valid_preds = target_model.predict(data.valid_set.nodes.tolist())
            ...
            print(f'  ==> The {unlearn_method} is trained, the result, acc: {res["accuracy"]}')
            ...
            for _ in tqdm(range(_num_behaviors), desc=f'At trial {t}, certifing'):
                ...
                p_star, p_post = surrogate.predict(data, device, target_nodes=v_star, return_posterior=True)
                target_preds, target_post = target_model.predict(v_star, return_posterior=True)
                used_token_nodes.extend(v_star)

                confidence_target[t][unlearn_method].append(utils.confidence(target_post))
                confidence_surrogate[t][unlearn_method].append(utils.confidence(p_post))
                ...
                if unlearn_method == 'retrain':
                    ...
                    adv_preds = unlearn_model.predict(adv_data, device, target_nodes=v_star)
                    ...
                    unlearn_model = unlearn.MEGU(
                        args.seed, adv_data.x, adv_data.adjacency_matrix().to_dense(), adv_data.y,
                        config, device, model_type='gcn', epochs=args.epochs, patience=args.patience,
                        edge_index=adv_data.edge_index, verbose=args.verbose,
                        alpha1=args.alpha1, alpha2=args.alpha2, kappa=args.kappa,
                    )
                    adv_res = unlearn_model.train()
                    adv_preds = unlearn_model.predict(v_star)
                ...
                if unlearn_method == 'retrain':
                    ...
                    neg_preds = neg_model.predict(neg_data, device, target_nodes=v_star)
                    neg_res = neg_model.evaluate(neg_data, device)
                else:
                    neg_model = copy.deepcopy(unlearn_model)
                    neg_res = neg_model.unlearn(perturbations)
                ...
                if unlearn_method == 'retrain':
                    ...
                    pos_preds = pos_model.predict(pos_data, device, target_nodes=v_star)
                ...
                # if _tn_count == args.num_target_nodes:
                #     tn += 1
                if (neg_preds != adv_preds).sum() == args.num_target_nodes:
                    tn += 1
                if (pos_preds == adv_preds).sum() != 0:
                    tp += 1
                print(f'At trial {t}, the prediction of {unlearn_method}:')
                print('token nodes:', v_star)
                print('perturbation:', ','.join([f'{e[1]}({data.y[e[1]]})' for e in e_star[0]]), ','.join([f'{e[1]}({data.y[e[1]]})' for e in e_star[1]]))
                print('benign preds:', p_star)
                print('adv_preds:', adv_preds)
                print('neg_preds:', neg_preds)
                print('pos_preds:', pos_preds)
                print('adv_acc:', adv_res['accuracy'])
                print('neg_acc:', neg_res['accuracy'])
                print('pos_acc:', pos_res['accuracy'])
                # print('num of retrain', num_retrains)
                print('-' * 80)

            # np.save(f'{unlearn_method}_preds_{args.dataset}_{ts}.npy', token_nodes_target_preds)
            # np.save(f'{unlearn_method}_posts_{args.dataset}_{ts}.npy', token_nodes_target_posts)
            # np.save(f'{unlearn_method}_surrogate_preds_{args.dataset}_{ts}.npy', token_nodes_surrogate_preds)
            # np.save(f'{unlearn_method}_surrogate_posts_{args.dataset}_{ts}.npy', token_nodes_surrogate_posts)

            result['trial'].append(t)
            result['method'].append(unlearn_method)
            result['m'].append(args.num_target_nodes)
            result['alpha1'].append(args.alpha1)
            result['alpha2'].append(args.alpha2)
            result['kappa'].append(args.kappa)
            result['TPR'].append(tp / _num_behaviors)
            result['TNR'].append(tn / _num_behaviors)
            ...
            result['confident score target'].append(np.mean(confidence_target[t][unlearn_method]))
            result['confident score surrogate'].append(np.mean(confidence_surrogate[t][unlearn_method]))
        ...
        print(f'trial {t} is done')
        print(f'  ==> The main result at {t}:')
        print(f'  ==> (alpha1, alpha2, kappa)', args.alpha1, args.alpha2, args.kappa)
        print(f'  ', trial_df.groupby(['method']).mean()[['m', 'TPR', 'TNR']])
    ...
    for t in range(args.num_trials):
        ...
        for unlearn_method in ['gnndelete']:
            ...
            if unlearn_method == 'eraser':
                ...
                target_train_preds = target_model.predict(data.train_set.nodes.tolist())
                target_valid_preds = target_model.predict(data.valid_set.nodes.tolist())
                target_test_preds = target_model.predict(data.test_set.nodes.tolist())
                print('!!!', torch.unique(torch.tensor(target_test_preds), return_counts=True))
            else:
                target_model = GNN(args, data.num_features, data.num_classes, surrogate=False, fix_weight=True)
                target_model.train(data, device)
                target_train_preds = target_model.predict(data, device, target_nodes=data.train_set.nodes.tolist())
            ...
            surrogate = GNN(args, data.num_features, data.num_classes, surrogate=False, bias=True, fix_weight=True)
            surrogate.train(surrogate_data, device)
            surr_res = surrogate.evaluate(data, device)
            # surrogate_filename = os.path.join('intermediate', args.dataset, f'surrogate{t}_{ts}_{unlearn_method}.pt')
            # surrogate.save_model(surrogate_filename)
            print(f' ==> The surrogate is trained, the result is {surr_res["accuracy"]}')

            _num_behaviors = 10
            tp, tn = 0, 0
            pos_token_node_acc, neg_token_node_acc = 0, 0
            used_token_nodes = []
            token_nodes_target_preds, token_nodes_target_posts = [], []
            token_nodes_surrogate_preds, token_nodes_surrogate_posts = [], []
            for _ in tqdm(range(_num_behaviors), desc=f'At trial {t}, certifing'):
                v_star, e_star, c_star = find_1pf(args, target_model, surrogate, data, device, verbose=False, used_token_nodes=used_token_nodes)
                p_star, p_post = surrogate.predict(data, device, target_nodes=v_star, return_posterior=True)
                target_preds, target_post = target_model.predict(v_star, return_posterior=True)
                ...
                _tp_count, _tn_count = 0, 0
                adv_acc_list, neg_acc_list, pos_acc_list = [], [], []
                for v, e, p, c in zip(v_star, e_star, target_preds, c_star):
                    adv_data = copy.deepcopy(data)
                    adv_data.add_edges(to_undirected(torch.tensor(e).t()))
                    ...
                    if unlearn_method == 'retrain':
                        ...
                        # for ee in e:
                        #     u, v = ee
                        #     for i in range(unlearn_model.num_shards):
                        #         if (u in unlearn_model.shards[i] and v not in unlearn_model.shards[i]) \
                        #         or (v in unlearn_model.shards[i] and u not in unlearn_model.shards[i]):
                        #             invalid_perturbations.append(ee)
                                    
                        # print('invalid perturbations:', invalid_perturbations, 'size:', len(invalid_perturbations))
                        adv_res = unlearn_model.train()
                        ...
                        unlearn_model = unlearn.MEGU(
                            args.seed, adv_data.x, adv_data.adjacency_matrix().to_dense(), adv_data.y,
                            config, device, model_type='gcn', epochs=args.epochs, patience=args.patience,
                            edge_index=adv_data.edge_index,
                            verbose=args.verbose,
                            alpha1=args.alpha1, alpha2=args.alpha2, kappa=args.kappa,
                        )
                        adv_res = unlearn_model.train()
                    ...
                    sub_perts = random.sample(e, min(int(len(e) * (1 - 0.2)), len(e)))
                    if unlearn_method == 'retrain':
                        pos_data = copy.deepcopy(adv_data)
                        pos_data.remove_edges(sub_perts)
...
def analysis_unlearning_methods(args):
    device = utils.get_device(args)
    ...
    for t in range(args.num_trials):
        ...
        args.num_target_nodes = 2
        # for unlearn_method in ['ceu', 'gnndelete', 'megu']:
        for unlearn_method in ['gnndelete']:
            if unlearn_method == 'eraser':
                target_model = unlearn.GraphEraser(
                    args.seed, data.x, data.adjacency_matrix().to_dense(), data.y,
            ...
            for _ in tqdm(range(_num_behaviors), desc=f'At trial {t}, certifing'):
                ...
                for v, e, c in zip(v_star, e_star, c_star):
                    candidates = []
                    for ee in e:
                        if ee[0] == v:
                            candidates.append(ee[1])
                        else:
                            candidates.append(ee[0])
                    if check_1pf(args, surrogate, neg_model, data, device, v, candidates, c, unlearn_method=unlearn_method):
                        num_tokens += 1
                
            result['trial'].append(t)
            result['method'].append(unlearn_method)
            result['m'].append(args.num_target_nodes)
            result['num tokens'].append(num_tokens)
...
def _majority_vote(preds):
    return np.where(np.sum(preds, axis=0) > preds.shape[0] / 2, 1, 0)

def verify_unlearning_mia(args):
    device = utils.get_device(args)
    if args.seed is not None:
        torch.manual_seed(args.seed)
        torch.cuda.manual_seed_all(args.seed)
    ...
    for t in range(args.num_trials):
        ...
        data = data_loader.load(args)
        target_model = _get_target_model(args, data, device)
        target_train_preds = target_model.predict(data, device, data.train_set.nodes.tolist())
        target_valid_preds = target_model.predict(data, device, data.valid_set.nodes.tolist())

        surrogate_data = copy.deepcopy(data)
        surrogate_data.train_set.y = torch.from_numpy(target_train_preds)
        surrogate_data.valid_set.y = torch.from_numpy(target_valid_preds)
        surrogate = GNN(args, surrogate_data.num_features, surrogate_data.num_classes, surrogate=False, bias=True, fix_weight=True)
        surrogate.train(surrogate_data, device)

        _num_behaviors = 50
        args.num_target_nodes = 2
        tp, tn = 0, 0
        for _ in tqdm(range(_num_behaviors), desc=f'At trial {t}, mia verifying'):
            v_star, e_star, c_star = find_1pf(args, target_model, surrogate, data, device, verbose=False)
            p_star = target_model.predict(data, device, target_nodes=v_star)
            ...
            mias = []
            for _ in range(20):
                mia, mia_res = steal_link(adv, data, adv_data, device)
                mias.append(mia)

            # verify by MIA
            mia_adv_preds_list = []
            for mia in mias:
                _adv_preds = mia_predict(mia, perturbations, adv, adv_data, device)
                mia_adv_preds_list.append(_adv_preds)
            mia_adv_preds = _majority_vote(np.concatenate(mia_adv_preds_list, axis=0)) 
            mia_adv_labels = [1] * len(perturbations)
            mia_adv_acc = np.sum(mia_adv_preds == mia_adv_labels) / len(mia_adv_labels)

            # Negative trial
            neg_data = copy.deepcopy(adv_data)
            neg_data.remove_edges(perturbations)
            neg = GNN(args, data.num_features, data.num_classes, surrogate=False, fix_weight=True)
            neg.train(neg_data, device)
            neg_preds = neg.predict(neg_data, device, target_nodes=v_star)
            mia_neg_preds_list = []
            for mia in mias:
                _neg_preds = mia_predict(mia, perturbations, neg, neg_data, device)
                mia_neg_preds_list.append(_neg_preds)
            mia_neg_preds = _majority_vote(np.concatenate(mia_neg_preds_list, axis=0))
            mia_neg_labels = [0] * len(perturbations)
            mia_neg_acc = np.sum(mia_neg_preds == mia_neg_labels) / len(mia_neg_labels)
            soundness = (neg_preds != adv_preds).sum() == args.num_target_nodes

            # Positive trial
            sub_perts = random.sample(perturbations, min(int(len(perturbations) * (1 - 0.2)), len(perturbations)))
            pos_data = copy.deepcopy(adv_data)
            pos_data.remove_edges(sub_perts)
            pos = GNN(args, data.num_features, data.num_classes, surrogate=False, fix_weight=True)
            pos.train(pos_data, device)
            pos_preds = pos.predict(pos_data, device, target_nodes=v_star)
            mia_pos_preds_list = []
            for mia in mias:
                _pos_preds = mia_predict(mia, perturbations, pos, pos_data, device)
                mia_pos_preds_list.append(_pos_preds)
            mia_pos_preds = _majority_vote(np.concatenate(mia_pos_preds_list, axis=0))
            mia_pos_labels = [0 if e in sub_perts else 1 for e in perturbations]
            mia_pos_acc = np.sum(mia_pos_preds == mia_pos_labels) / len(mia_pos_labels)
            # if (pos_preds == adv_preds).sum() > 0:
            #     tp += 1
            necessity = (pos_preds == adv_preds).sum() > 0

            result['trial'].append(t)
            result['mia acc'].append(mia_res['accuracy'])
            result['mia recall'].append(mia_res['recall'])
            result['mia precision'].append(mia_res['precision'])
            result['mia f1'].append(mia_res['f1'])
            result['adv acc'].append(mia_adv_acc)
            result['neg acc'].append(mia_neg_acc)
            result['pos acc'].append(mia_pos_acc)
            result['mia tp'] = 1 if mia_pos_acc.sum() != 0 else 0
            result['mia tn'] = 1 if mia_neg_acc.sum() == 0 else 0
            result['ours tp'] = 1 if necessity else 0
            result['ours tn'] = 1 if soundness else 0
            result['adv preds'].append(adv_preds)
            result['neg preds'].append(neg_preds)
            result['pos preds'].append(pos_preds)
            result['mia adv preds'].append(mia_adv_preds)
            result['mia neg preds'].append(mia_neg_preds)
            result['mia pos preds'].append(mia_pos_preds)

            

    df = pd.DataFrame(result)
    result_filename = f'verfiy_mia_{args.dataset}_{ts}.csv'
    df.to_csv(os.path.join('result', result_filename))
    print('-' * 80)
    print('Verifing MIA is done, the result file is saved to', result_filename)
    print(' ==> MIA TPR:', df['mia tp'].mean())
    print(' ==> MIA TNR:', df['mia tn'].mean())
    print(' ==> Ours TPR:', df['ours tp'].mean())
    print(' ==> Ours TNR:', df['ours tn'].mean())
    print('-' * 80)

def verify_unlearning_mia_random_edges(args):
    device = utils.get_device(args)
    if args.seed is not None:
        torch.manual_seed(args.seed)
        torch.cuda.manual_seed_all(args.seed)
    ...
    for t in range(args.num_trials):
        ...
        for _ in tqdm(range(_num_behaviors), desc=f'At trial {t}, mia verifying'):
            ...
            neg_edge_index = negative_sampling(
                data.edge_index, data.num_nodes, num_neg_samples=10,
            )
            perturbations = list(map(tuple, neg_edge_index.t().tolist()))
            print('Perturbations:', perturbations)
            adv_data = copy.deepcopy(data)
            adv_data.add_edges(to_undirected(torch.tensor(perturbations).t()))
            ...
            # verify by MIA
            mia_adv_preds = mia_predict(mia, perturbations, adv, adv_data, device)
            mia_adv_labels = [1] * len(perturbations)
            mia_adv_acc = np.sum(mia_adv_preds == mia_adv_labels) / len(mia_adv_labels)
            ...
            neg = GNN(args, data.num_features, data.num_classes, surrogate=False, fix_weight=True)
            neg.train(neg_data, device)
            mia_neg_preds = mia_predict(mia, perturbations, neg, neg_data, device)
            mia_neg_labels = [0] * len(perturbations)
            mia_neg_acc = np.sum(mia_neg_preds == mia_neg_labels) / len(mia_neg_labels)
            ...
            pos = GNN(args, data.num_features, data.num_classes, surrogate=False, fix_weight=True)
            pos.train(pos_data, device)
            mia_pos_preds = mia_predict(mia, perturbations, pos, pos_data, device)
            mia_pos_labels = [0 if e in sub_perts else 1 for e in perturbations]
            mia_pos_acc = np.sum(mia_pos_preds == mia_pos_labels) / len(mia_pos_labels)
    ...
    df = pd.DataFrame(result)
    result_filename = f'verfiy_mia_random_{args.dataset}_{ts}.csv'
    df.to_csv(os.path.join('result', result_filename))
    print('-' * 80)
    print('Verifing MIA is done, the result file is saved to', result_filename)
    print(' ==> MIA TPR:', df['mia tp'].mean())
...
def estimate_p_q_unlearn_methods(args):
    device = utils.get_device(args)
    # args.num_target_nodes = 10
    ...
    for t in range(args.num_trials):
        ...
        # for unlearn_method in ['eraser', 'ceu', 'gnndelete']:
        for unlearn_method in ['gnndelete']:
            if unlearn_method == 'eraser':
                target_model = unlearn.GraphEraser(
                    args.seed, data.x, data.adjacency_matrix().to_dense(), data.y, 
            ...
            num_neccessity = 0
            num_correctness = 0
            for v, pred, perturbations, attack_label in tqdm(zip(v_star, p_star, e_star, c_star), total=len(v_star), desc=f'estimating p and q'):
                adv_data = copy.deepcopy(data)
                adv_data.add_edges(to_undirected(torch.tensor(perturbations).t()))
                ...
                if neg_pred != adv_pred:
                    num_correctness += 1
                if not check_incompleteness_unlearn(args, v, adv_pred, adv_data, perturbations, device, unlearn_method, unlearn_model):
                    num_neccessity += 1

            result['trial'].append(t)
            result['method'].append(unlearn_method)
            result['neccessity'].append(num_neccessity)
            result['q'].append(num_neccessity / (len(v_star)))
            result['correctness'].append(num_correctness)
            result['p'].append(num_correctness / len(v_star))
    ...
    result_filename = f'estimate_unlearn_method_{args.dataset}_{ts}.csv'
    df.to_csv(os.path.join('result', result_filename))
    print('Estimation of p and q is done, the result file is saved to', result_filename)
...
def find_guaranteed_m():
    for method in ['ceu', 'gnndelete', 'eraser']:
        for m in range(1, 5):
            p_tp = 1 - (1 - unlearn_q[method])** m
            p_tn = unlearn_p[method] ** m
            print(f'For method {method}, m={m}, p_tp={p_tp}, p_tn={p_tn}')
...
def vary_target_model(args):
    ...
    confident_score_target, confidence_surrogate = {}, {}
    accuracy_target, accuracy_surrogate = {}, {}
    ...
    for t in range(args.num_trials):
        ...
        data = data_loader.load(args)
        confident_score_target[t], confidence_surrogate[t] = {}, {}
        accuracy_target[t], accuracy_surrogate[t] = {}, {}

        for target in ['gcn']:
            confident_score_target[t][target], confidence_surrogate[t][target] = [], []
            accuracy_target[t][target], accuracy_surrogate[t][target] = [], []
            ...
            for _ in tqdm(range(_num_behaviors), desc=f'At trial {t}'):
                ...
                p_star, target_post = target_model.predict(data, device, target_nodes=v_star, return_posterior=True)
                res = target_model.evaluate(data, device)
                accuracy_target[t][target].append(res['accuracy'])
                confident_score_target[t][target].append(utils.confidence(target_post))

                _, surrogate_post = surrogate.predict(data, device, target_nodes=v_star, return_posterior=True)
                res = surrogate.evaluate(data, device)
                confidence_surrogate[t][target].append(utils.confidence(surrogate_post))
                accuracy_surrogate[t][target].append(res['accuracy'])
            ...
            result['target confidence'].append(np.mean(confident_score_target[t][target]))
            result['surrogate confidence'].append(np.mean(confidence_surrogate[t][target]))
    ...
    np.save(os.path.join(f'target_model_accuracy_{ts}_{args.dataset}.npy'), accuracy_target)
    np.save(os.path.join(f'target_model_confidence_{ts}_{args.dataset}.npy'), confident_score_target)
    np.save(os.path.join(f'surrogate_accuracy_{ts}_{args.dataset}.npy'), accuracy_surrogate)
    np.save(os.path.join(f'surrogate_confidence_{ts}_{args.dataset}.npy'), confidence_surrogate)


def confidence_target_model(args):
    device = utils.get_device(args)
    if args.seed is not None:
        torch.manual_seed(args.seed)
        np.random.seed(args.seed)
    ...
    for t in range(args.num_trials):
        ...
        _, target_test_post = target_model.predict(data, device, target_nodes=data.test_set.nodes.tolist(), return_posterior=True)
        _, surrogate_test_post = surrogate.predict(data, device, target_nodes=data.test_set.nodes.tolist(), return_posterior=True)

        # Compute the confidence
        target_confidence = utils.confidence(target_test_post)
        surrogate_confidence = utils.confidence(surrogate_test_post)

        # args.num_target_nodes = 10
        # v_star, e_star, c_star = find_1pf(args, target_model, surrogate, data, device, verbose=True)
        # target_preds, target_post = target_model.predict(data, device, target_nodes=v_star, return_posterior=True)
        # surrogate_preds, surrogate_post = surrogate.predict(data, device, target_nodes=v_star, return_posterior=True)
        # target_1pf_confidence = utils.confidence(target_post)
        # surrogate_1pf_confidence = utils.confidence(surrogate_post)

        result['trial'].append(t)
        result['target accuracy'].append(target_res['accuracy'])
        result['surrogate accuracy'].append(surrogate.evaluate(surrogate_data, device)['accuracy'])
        # result['target confidence'].append(np.mean(target_1pf_confidence))
        # result['surrogate confidence'].append(np.mean(surrogate_1pf_confidence))
        result['all confidence target'].append(np.mean(target_confidence))
        result['all confidence surrogate'].append(np.mean(surrogate_confidence))

        # for i in range(10):
        #     print(f'  => target confidence: {target_1pf_confidence[i]}, surrogate confidence: {surrogate_1pf_confidence[i]}')
        #     print(f'  => posterior', target_post[i], surrogate_post[i])
        #     adv_pred = _adv_train_predict(args, data, e_star[i], [v_star[i]], device)[0]
        #     print(f'  => adv pred: {adv_pred}, target pred: {target_preds[i]}, surrogate pred: {surrogate_preds[i]}')
        #     for j in range(5):
        #         challenge_pred = _adv_train_predict(args, data, [e_star[i][j]], [v_star[i]], device)[0]
        #         print(f'  => challenge pred: {challenge_pred}, challenge label: {c_star[i]}')
        #     print(f'  => challenge edges: {e_star[i]}')
    ...
    result_filename = f'confidence_target_model_{args.dataset}_{ts}.csv'
    df.to_csv(os.path.join('result', result_filename))
    print('Confidence of the target model is saved to', result_filename)
    print('result:', df, df.mean())


def confidence_surrogate(args):
    device = utils.get_device(args)
    if args.seed is not None:
        torch.manual_seed(args.seed)
        np.random.seed(args.seed)
    ...
    for t in range(args.num_trials):
        ...
        idx_nodes_whole = [surrogate_data.partial_to_original[v] for v in surrogate_data.train_set.nodes.tolist()]
        whole_to_partial = {surrogate_data.partial_to_original[v]: v for v in surrogate_data.train_set.nodes.tolist()}
        target_preds = target_model.predict(data, device, target_nodes=idx_nodes_whole)
        surrogate_data.train_set.labels = target_preds
        ...
        for surrogate_type in ['simplified', '4-neurons', '8-neurons', 'standard']:
            ...
            args.num_target_nodes = 10
            v_star, e_star, c_star = find_1pf(args, target_model, surrogate, data, device, verbose=False, surrogate_data=surrogate_data)
            _, target_post = target_model.predict(data, device, target_nodes=v_star, return_posterior=True)

            # v_star_prime = [whole_to_partial[v] for v in v_star]
            _, surrogate_post = surrogate.predict(data, device, target_nodes=v_star, return_posterior=True)

            """ Compute the confidence
                1. the max in the posterior - the second max in the posterior
                2. entropy of the posterior
            """
            target_confidence = []
            surrogate_confidence = []
            target_entropy, surrogate_entropy = [], []
            for t_post, s_post in zip(target_post, surrogate_post):
                target_confidence.append(t_post.max() - t_post[t_post != t_post.max()].max())
                surrogate_confidence.append(s_post.max() - s_post[s_post != s_post.max()].max())
                target_entropy.append(-np.sum(t_post * np.log(t_post)))
                surrogate_entropy.append(-np.sum(s_post * np.log(s_post)))
 
            result['trial'].append(t)
            result['target'].append(surrogate_type)
            result['target confidence'].append(np.mean(target_confidence))
            result['surrogate confidence'].append(np.mean(surrogate_confidence))
            result['target entropy'].append(np.mean(target_entropy))
            result['surrogate entropy'].append(np.mean(surrogate_entropy))
    ...
    result_filename = f'confidence_target_model_{ts}_{args.dataset}_{ts}.csv'
    df.to_csv(os.path.join('result', result_filename))
    print('Confidence of the target model is saved to', result_filename)
...
def m_guarantee():
    for alpha in [0.65, 0.7, 0.75, 0.8, 0.85, 0.9]:
        for beta in [0.65, 0.7, 0.75, 0.8, 0.85, 0.9]:
            # for edge_unlearn in ['eraser', 'ceu', 'gnndelete']:
            for edge_unlearn in ['ceu']:
                m = _calc_m(alpha, beta, unlearn_p[edge_unlearn], unlearn_q[edge_unlearn])
                if m is None:
                    print(f'There is no m for {edge_unlearn}, alpha={alpha}, beta={beta}')
                else:
                    print(f'{edge_unlearn} => m={m}, alpha={alpha}, beta={beta}')
...
def _generate_mia_features(edges, labels, posterior, features, node2idx):
    x = build_features(edges, posterior, node_features=features, node2idx=node2idx)
    y = np.array(labels)
...
def mia_evaluation(mia, x_test, y_test):
...
def mia_predict(mia, edges, target_model, data, device):
    features = data.x.numpy()
    _, posterior = target_model.predict(data, device, return_posterior=True)

    x_test = build_features(edges, posterior, node_features=features, node2idx=None)
    x_test = np.nan_to_num(x_test)
    x_test = x_test.astype('float')
    ss = StandardScaler()
    x_test = ss.fit_transform(x_test)
...
def steal_link(target_model, data, adv_data, device):
    # train a MIA model
    features = data.x.numpy()
    # edge_index = data.edge_index.to(device)
    _edge_index = utils.to_directed(data.edge_index)

    _, posterior = target_model.predict(adv_data, device, return_posterior=True)

    random_idx = random.sample(range(_edge_index.size(1)), int(_edge_index.size(1) * 0.4))
    ...
    train_edges, test_edges, train_labels, test_labels = train_test_split(
        train_edges, train_labels, test_size=0.5, random_state=args.seed)
    x_train, y_train = _generate_mia_features(
        train_edges, train_labels, posterior, features, node2idx=None)
    x_test, y_test = _generate_mia_features(
        test_edges, test_labels, posterior, features, node2idx=None
    )
    mlp = MIA(x_train, y_train)
    res = mia_evaluation(mlp, x_test, y_test)
    return mlp, res
...
def _jaccard_similairty(U, V):
    M_11 = np.sum(U * V, axis=1)
    M_01 = np.sum(U * (1 - V), axis=1)
    M_10 = np.sum((1 - U) * V, axis=1)
    return M_11 / (M_01 + M_10 + M_11)

def _od_node_feature(data, v):
    neighbors = list(data.adj_list[v])
    _x = np.zeros(5)
    uni_classes, counts = np.unique(data.y[neighbors].cpu().numpy(), return_counts=True)
...
def _od_feature(data, edges):
    features = np.zeros((len(edges), 10))
    for i, (a, b) in enumerate(edges):
        features[i, :5] = _od_node_feature(data, a)
        features[i, 5:] = _od_node_feature(data, b)

    return features


def batch_cosine_similarity(vector, matrix):
    # Normalize the vectors
    vector_norm = vector / np.linalg.norm(vector)
    matrix_norm = matrix / np.linalg.norm(matrix, axis=1, keepdims=True)
    
    # Compute cosine similarity for all rows in the matrix
    similarities = np.dot(matrix_norm, vector_norm)
    return similarities


def analysis_challenge_edge(args):
    device = utils.get_device(args)
    args.seed = random.randint(0, 1e+5)
    ...
    direct_edge_index = utils.to_directed(data.edge_index)
    random_edges = random.sample(range(direct_edge_index.size(1)), int(0.5 * direct_edge_index.size(1)))
    random_edges = direct_edge_index[:, random_edges].t().cpu().tolist()
    ...
    neg_random_edges = negative_sampling(
        adv_data.edge_index, num_neg_samples=len(direct_edge_index)
    ).t().cpu().tolist()

    # print('The number of perturbations:', len(perturbations), perturbations[:5], '...')
    # print('The number of random edges:', len(random_edges), random_edges[:5], '...')
    # print('The number of negative random edges:', len(neg_random_edges), neg_random_edges[:5], '...')

    pert_edge_index = torch.tensor(perturbations).t()
    random_edge_index = torch.tensor(random_edges).t()
    neg_random_edge_index = torch.tensor(neg_random_edges).t()
    # both_edge_index = torch.cat([random_edge_index, neg_random_edge_index], dim=1)

    _, logits = target_model.predict(data, device, return_logit=True)
    logits = torch.from_numpy(logits)

    _, adv_logits = adv.predict(adv_data, device, return_logit=True)
    adv_logits = torch.from_numpy(adv_logits)

    cos = torch.nn.CosineSimilarity()
    adv_pert_similarity = cos(adv_logits[pert_edge_index[0]], adv_logits[pert_edge_index[1]])
    adv_random_edge_similarity = cos(adv_logits[direct_edge_index[0]], adv_logits[direct_edge_index[1]])
    adv_neg_random_edge_similarity = cos(adv_logits[neg_random_edge_index[0]], adv_logits[neg_random_edge_index[1]])
    # adv_both_edge_similarity = cos(adv_logits[both_edge_index[0]], adv_logits[both_edge_index[1]])

    print('The adv similarity of perturbations:', adv_pert_similarity.mean().item())
    print('The adv similarity of random edges:', adv_random_edge_similarity.mean().item())
    print('The adv similarity of negative random edges:', adv_neg_random_edge_similarity.mean().item())
    # print('The adv similarity of both random edges:', adv_both_edge_similarity.mean().item())


    pert_features = _od_feature(adv_data, pert_edge_index.t().cpu().tolist())
    other_features = _od_feature(adv_data, direct_edge_index.t().cpu().tolist())
    # od_feature_similarity = cos(torch.from_numpy(pert_features), torch.from_numpy(other_features))
    od_feature_similarity = np.mean([batch_cosine_similarity(pf, other_features) for pf in pert_features])
    print('The OD feature similarity of perturbations:', od_feature_similarity)
    ...
    # v_star = [v for v in v_star if v in mapping_idx]
    # e_star = [[[mapping_idx[u], mapping_idx[v]] for u, v in e] for e in e_star]
    # detector = GraphGenDetect(subdata, v_star, e_star, device)
    # res = detector.detect_all()

    challenge_edges = [tuple(e) for es in e_star for e in es]
    # edge2occurrence = defaultdict(int)
    # challenge_edge2occurrence = defaultdict(int)
    # for _, subset, edge_index in detector.subgraphs:
    #     for e in edge_index.t().cpu().tolist():
    #         idx2node = {idx: v for idx, v in enumerate(subset)}
    #         edge2occurrence[tuple(sorted(e))] += 1
    #         u, v = idx2node[e[0]], idx2node[e[1]]
    #         if (u, v) in challenge_edges or (v, u) in challenge_edges:
    #             challenge_edge2occurrence[tuple(sorted([u, v]))] += 1
    # print('25th percentile:', np.percentile(list(edge2occurrence.values()), 25))
    # print('25th percentile of challenge edges:', np.percentile(list(challenge_edge2occurrence.values()), 25))

    adv_subdata = copy.deepcopy(data)
    adv_subdata.add_edges(to_undirected(torch.tensor(challenge_edges).t()))

    avg_degree_all_edges = []
    avg_degree_challenge_edges = []
    for edge in adv_subdata.edges:
        sub, _, _, _ = k_hop_subgraph([edge[0], edge[1]], 2, data.edge_index)
        avg_degree = np.mean([adv_subdata.degree(v) for v in sub])
        avg_degree_all_edges.append(avg_degree)
        if edge in challenge_edges or (edge[1], edge[0]) in challenge_edges:
            avg_degree_challenge_edges.append(avg_degree)
            
    print(f'25th percentile of all edge degree: {np.percentile(avg_degree_all_edges, 25)}', np.mean(avg_degree_all_edges))
    print(f'25th percentile of challenge edge degree: {np.percentile(avg_degree_challenge_edges, 25)}', np.mean(avg_degree_challenge_edges))
    print(f'10th percentile of all edge degree: {np.percentile(avg_degree_all_edges, 10)}', np.mean(avg_degree_all_edges))
    print(f'10th percentile of challenge edge degree: {np.percentile(avg_degree_challenge_edges, 10)}', np.mean(avg_degree_challenge_edges))
...
if __name__ == '__main__':
    ...
    #     steal_link(args)
        ...
        verify_unlearning_mia_random_edges(args)
    elif args.task == 'analysis-challenge-edge':
        analysis_challenge_edge(args)