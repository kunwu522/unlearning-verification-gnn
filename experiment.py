...
def _unlearn_train_predict(args, data, perturbations, target_nodes, device, type=None):
    ...
    if type is not None:
        if type == '1-layer':
            ...
            args.hidden_size = 16
            unlearn = GNN(args, data.num_features, data.num_classes, surrogate=False, fix_weight=False, layer3=True)
        else:
            raise ValueError(f'Unknown type {type}')
    else:
    ...
    return unlearn_pred, verify_time
...
def find_1pf(args, target, surrogate, data, device, num_layer=2, verbose=False, surrogate_data=None):
    weights = copy.deepcopy([p.detach().numpy() for p in surrogate.parameters()])
...
def performance_comparison(args):
    ...
    for t in range(args.num_trials):
        ...
        _num_behaviors = 50
        for method in ['sga', 'fga', 'rnd', 'nettack', 'ours']:
            tp, tn = 0, 0
            ...
            result['TPR'].append(tp / _num_behaviors)
            result['TNR'].append(tn / _num_behaviors)
            result['accuracy loss'].append(acc_loss)
...
def targeted_attack(surrogate_data, method, num_perts, trial, ts, device): 
    ...
    # surrogate_filename = os.path.join('intermediate', args.dataset, f'surrogate{trial}_{ts}_{method}.pkl')
...
def composition(args):
    ...
    for t in range(args.num_trials):
        ...
        if args.gcn_type == '1-layer':
            ...
            args.condidate_size = 1
            args.hidden_size = 16
            target_model = GNN(args, data.num_features, data.num_classes, surrogate=False, fix_weight=False, layer3=True)
        else:
        ...
        verifier = CertifiedFragilenessLazy(args, weights, data, verbose=False, num_layer=num_layer)
        v_star, e_star, c_star = [], [], []
        t0 = time.time()
        pbar = tqdm(total=args.num_target_nodes, desc=f'At trial {t}, certifing')
        iter_count = 0
        ...
        for v, e, p, c in tqdm(zip(v_star, e_star, p_star, c_star), total=len(v_star), desc=f'At trial {t}, verifying'):
            tp, tn = 0, 0
            fp, fn = 0, 0
            ...
            adv_data = copy.deepcopy(data)
            adv_data.add_edges(to_undirected(torch.tensor(e).t()))
            # necessity, _ = check_incompleteness(args, v, p, adv_pred, adv_data, e, device, args.gcn_type) 
 
            # unlearn_pred = _unlearn_train_predict(args, data, e, [v], device)
            # if len(successed_perts) > 1:
            #     adv_data = copy.deepcopy(data)
            #     adv_data.add_edges(to_undirected(torch.tensor(successed_perts).t()))
            #     necessity, _ = check_incompleteness(args, v, p, adv_pred, adv_data, successed_perts, device, args.gcn_type) 
            # else:
            #     necessity = -1
            neg_pred, _ = _unlearn_train_predict(args, adv_data, e, [v], device, args.gcn_type)
            if neg_pred[0] != adv_pred:
                tn = 1
            else:
                fp = 1
            sub_perts = random.sample(e, max(int(len(e) * (1 - 0.2)), 1))
            pos_pred, _ = _unlearn_train_predict(args, adv_data, sub_perts, [v], device, args.gcn_type)
            if pos_pred[0] == adv_pred:
                tp = 1
            else:
                fn = 1
            ...
            # result['verify time'].append(time.time() - t0)
            # result['soundness'].append(1 if adv_pred != p else 0)
            # result['necessity'].append(necessity)

        _df = pd.DataFrame(result)
        _df = _df[_df['trial'] == t]
        _df.to_csv(os.path.join('intermediate', f'composition{t}_{ts}_{args.dataset}_{args.gcn_type}.csv'))
        print('-' * 80)
        print(f'  ==> The main result at {t}:')
        # print(f'  ', _df.groupby(['gcn_type', 'binary']).sum()[['soundness', 'necessity']])
...
def vary_surrogate(args):
    _num_behaviors = 50
    args.num_target_nodes = _calc_m(_alpah, _p[args.dataset])
...
def vary_unlearning(args):
    ...
    for t in range(args.num_trials):
        ...
        weights = copy.deepcopy([p.detach().numpy() for p in surrogate.parameters()])
        verifier = CertifiedFragilenessLazy(args, weights, surrogate_data, verbose=False)
        v_star, e_star, c_star = [], [], []
        pbar = tqdm(total=args.num_target_nodes, desc=f'At trial {t}, certifing')
        while len(v_star) < args.num_target_nodes:
            # sample a node
            ...
            if v in v_star:
                continue
            pred, post = surrogate.predict(data, device, target_nodes=[v], return_posterior=True)
            c = _second_best_labels(post)[0]
            v, perturbation, c = certify_fragile_with_perturbations((v, pred[0], c), args, verifier, data)
            if perturbation is not None:
                v_star.append(v)
                e_star.append(perturbation)
                c_star.append(c)
                pbar.update(1)
            pbar.set_description(f'At trial {t}, after certifing {len(v_star)} nodes')
        pbar.close()
        ...
        #     config, device, model='gcn'
        # )
        # res = target_model.train()
        # print('res:', res)
        # target_train_preds = target_model.predict(data.train_set.nodes.tolist())

        history_pred = defaultdict(list)
        for unlearn_method in ['retrain', 'ceu', 'eraser']:
            # if unlearn_method == 'eraser':
            #     unlearn_model = unlearn.GraphEraser(
            #         args.seed, data.x, data.adjacency_matrix().to_dense(), data.y, 
            #         config, device, model='gcn', epochs=args.epochs, patience=args.patience
            #     )
            #     res = unlearn_model.train()
            #     target_train_preds = unlearn_model.predict(data.train_set.nodes.tolist())
            ...
            # unlearn_model_filename = os.path.join('intermediate', args.dataset, f'unlearn{t}_{ts}_{unlearn_method}.pkl')
            # with open(unlearn_model_filename, 'wb') as fp:
            #     pickle.dump(unlearn_model, fp)
            ...
            # v_star_filename = os.path.join('intermediate', args.dataset, f'v_star{t}_{ts}_{unlearn_method}.pkl')
            # e_star_filename = os.path.join('intermediate', args.dataset, f'e_star{t}_{ts}_{unlearn_method}.pkl')
            # c_star_filename = os.path.join('intermediate', args.dataset, f'c_star{t}_{ts}_{unlearn_method}.pkl')
            # with open(v_star_filename, 'wb') as fp:
            #     pickle.dump(v_star, fp)
            # with open(e_star_filename, 'wb') as fp:
            #     pickle.dump(e_star, fp)
        ...
        # for unlearn_method in ['retrain', 'eraser', 'ceu']:
            tp, tn, fp, fn = 0, 0, 0, 0
            for v, e, pred in tqdm(zip(v_star, e_star, p_star), total=len(v_star), desc=f'Trial {t}, method {unlearn_method}, evaluating'):
                adv_data = copy.deepcopy(data)
                adv_data.add_edges(to_undirected(torch.tensor(e).t()))
                # adv_adj = adv_data.adjacency_matrix().to_dense()

                history_pred['method'].append(unlearn_method)
                history_pred['node'].append(v)
                history_pred['pred'].append(pred)
                ...
                if unlearn_method == 'retrain':
                    ...
                    # unlearn_model.adj = adv_adj.to(device)
                    # unlearn_model.train()
                ...
                history_pred['adv pred'].append(adv_pred)


                if unlearn_method == 'retrain':
                    neg_pred, _ = _unlearn_train_predict(args, adv_data, e, [v], device)
                else:
                ...
                history_pred['tn pred'].append(neg_pred)

                sub_perts = random.sample(e, min(int(len(e) * (1 - 0.2)), len(e)))
                if unlearn_method == 'retrain':
                    pos_pred, _ = _unlearn_train_predict(args, adv_data, sub_perts, [v], device)
                else:
                ...
                if pos_pred == adv_pred:
                    ...
                    fn += 1
                history_pred['tp pred'].append(pos_pred)
        ...
        print(f'trial {t} is done')
        print(f'  ==> The main result at {t}:')
        print(f'  ', trial_df.groupby(['method']).mean()[['tp', 'tn', 'fp', 'fn']])
...
def estimate_p1_p2(args):
    ...
    for t in range(args.num_trials):
        ...
        for v, pred, perturbations, attack_label in tqdm(zip(v_star, p_star, e_star, c_star), total=len(v_star), desc=f'estimating p1 and p2'):
            ...
            #     if adv_pred != pred:
            #         p_count += 1
            # result['p count'].append(p_count)
            # result['p'].append(p_count/len(perturbations))
            ...
            # is_incomplete, _failed_pert = check_incompleteness(args, v, pred, adv_pred, adv_data, perturbations, device)
            # if is_incomplete:
            #     result['incomplete prediction'].append(pred)
            #     result['incomplete'].append(1)
            #     result['failed perturbation'].append(_failed_pert)
...
def empirical_vs_theoretical(args):
    ...
    for t in range(args.num_trials):
        ...
        for p in [0.7, 0.75, 0.8, 0.85, 0.9, 0.95]:
            ...
            result['p'].append(p)
            result['tp'].append(tp)
            result['tpr'].append(tp / len(v_star))

    df = pd.DataFrame(result)
    result_filename = f'empirical_vs_theoretical_{ts}.csv'
    df.to_csv(os.path.join('result', result_filename))
    print('-' * 80)
    print(df.groupby(['m', 'p']).mean())
...
def varying_num_edges(args):
    ...
    df = pd.DataFrame(result)
    result_filename = f'varying_num_edges_{ts}.csv'
    df.to_csv(os.path.join('result', result_filename))
...
def varying_num_edges_baselines(args):
    ...
    for t in range(args.num_trials):
        ...
        for num_pert_edges in [5, 10, 15, 20, 25]:
            ...
            for method in ['sga', 'fga', 'rnd', 'nettack']:
                tp, tn = 0, 0
...
def varying_incomplete_ratio(args):
    ...
    for t in range(args.num_trials):
        ...
        _num_behaviors = 50
        for incomplete_ratio in [0.2, 0.4, 0.6, 0.8]:
            tp, tn = 0, 0
            for _ in tqdm(range(_num_behaviors), desc=f'At trial {t}, incomplete_ratio {incomplete_ratio}'):
                v_star, e_star, c_star = find_1pf(args, target_model, surrogate, data, device)
                p_star = target_model.predict(data, device, target_nodes=v_star)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-perts', dest='num_perts', type=int, default=10)
