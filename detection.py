...
class OutlierDetector(object):
    ...
    def detect(self):
        ...
        for challenge_edges in tqdm(self.e_star, desc='Outlier detecting'):
            ...
            num_edges = adv_data.edge_index.shape[1]
            train_features = np.zeros((num_edges, 10))
            for i, (a, b) in enumerate(adv_data.edge_index.t().cpu().tolist()):
...
class ProximityDetector(object):
    ...
    def _train_gcn(self, model):
        train_loader = DataLoader(self.data.train_set, batch_size=512, shuffle=False)
        valid_loader = DataLoader(self.data.valid_set, batch_size=1024)
    ...
    def _compute_score(self, model, v, es):
        adv_data = copy.deepcopy(self.data)
    ...
    def detect_all(self):
        ...
        for es in tqdm(self.e_star, total=len(self.v_star), desc='PD detecting'):
            u, v = es[0], es[1]
            p1_u, p2_u = self._compute_score(surrogate, v, es)
            p1_v, p2_v = self._compute_score(surrogate, v, es)
