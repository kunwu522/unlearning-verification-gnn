...
class Nettack:
    ...
    def __init__(self, adj, X_obs, z_obs, num_classes, W1, W2, u, verbose=False, 
        ...
        # A_UL
        if adj_ul is not None:
            self.adj_ul = adj_ul.copy().tolil()
        else:
            self.adj_ul = None
    ...
    def strongest_wrong_class(self, logits):
        ...
        if self.target_prediction is None:
            label_onehot = np.eye(self.K)[self.label_u]
        else:
            label_onehot = np.eye(self.K)[self.target_prediction]
        return (logits - 1000*label_onehot).argmax()
    ...
    def struct_score(self, a_hat_uv, XW):
        ...
        # prob = softmax(logits.todense(), axis=1)
        if self.target_prediction is None:
            label_onehot = np.eye(XW.shape[1])[self.label_u]
        else:
            label_onehot = np.eye(XW.shape[1])[self.target_prediction]

        if self.target_label is not None:
            best_wrong_class_logits = np.expand_dims(np.asarray(logits.todense()[:, self.target_label]).squeeze(), 1)
        else:
            best_wrong_class_logits = (logits - 1000 * label_onehot).max(1)

        # best_wrong_class_logits = (logits - 1000 * label_onehot).max(1)
        # best_wrong_class_logits = np.expand_dims((prob - 1 * label_onehot).max(1), 1) 
        if self.target_prediction is None:
            logits_for_correct_class = logits[:,self.label_u]
            # logits_for_correct_class = prob[:,self.label_u]
        else:
            logits_for_correct_class = logits[:, self.target_prediction]
            # logits_for_correct_class = prob[:, self.target_prediction]
    ...
    def attack_surrogate(self, n_perturbations, perturb_structure=True, perturb_features=False,
        ...
        if self.target_prediction is None:
            ...
            surrogate_losses = [logits_start[self.target_prediction] - logits_start[best_wrong_class]]

        # print(f'The best wrong class is {best_wrong_class}, target prediction is {self.target_prediction}, target label is {self.target_label}')
        ...
        for _ in range(n_perturbations):
            ...
            if perturb_structure:
                ...
                # if self.verbose:
                #     print('struct_scores', struct_scores[:n_perturbations])
                """
                ...
                if self.add_edge_only:
                    for ix in np.asarray(struct_scores).squeeze().argsort():
                        _edge = filtered_edges_final[ix]
                        if self.adj[tuple(_edge)] == 0:
                            best_edge_ix = ix
                            best_edge_score = struct_scores[ix]
                            best_edge = filtered_edges_final[ix]
                            break
                else:
            ...
            # if self.epsilon >= 0 and best_edge_score.item() < (- self.epsilon) and len(self.structure_perturbations) > 1:
            #     break
            # if self.epsilon >= 0 and best_edge_score.item() < (- self.epsilon):
            #     break
