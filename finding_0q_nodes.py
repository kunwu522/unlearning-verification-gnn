...
def check_necessity(args):
    ...
    for t in range(args.num_trials):
        ...
        for target_node, prediction, label, post in tqdm(zip(target_nodes, predictions, labels, posteriors), total=len(target_nodes), desc=f'Trial {t}'):
            ...
            boundary_score = utils.boundary_score(post)
            feat_norm = torch.norm(data.x[target_node])
            ...
            if num_of_flip == 0:
                ...
                result['degree'].append(data.degree(target_node))
                ...
                result['boundary score'].append(boundary_score)
                result['feature norm'].append(feat_norm)

    
    df = pd.DataFrame(result)

    result_path = os.path.join('result', f'uninfected_{args.dataset}_{args.sampler}{args.num_target_nodes}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sampler', type=str, default='random', help='How to sample target nodes, distance|boundary')
