...
def preprocess_adj(adj, ):
    ...
    if sp.isspmatrix(adj):
        ...
        adj_tilde = torch.eye(N, device=adj.device) + adj
        degs_inv = torch.pow(adj_tilde.sum(dim=0), -0.5)
        adj_norm = torch.multiply(torch.multiply(adj_tilde, degs_inv[None,:]), degs_inv[:,None])
