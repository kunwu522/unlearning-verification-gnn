...
class GCNNodeCertification:
    def __init__(self, i_th, A, twohop_nbs, onehop_ixs_th, onehop_nbs, X, weights,
        ...
        self.A_upper = self._naive_entrywise_upper_bounds()
        ...
        self.A_lower_th = self._naive_entrywise_lower()
        ...
        if self.bias:
            ...
            self.H2_hat_cvx = self.An_cvx @ self.XW
    ...
    def _naive_entrywise_upper_bounds(self):
        # A_upper = np.multiply(self.An_th_th, self.A_mask_th)
        # A_upper = np.zeros_like(self.An_th_th)
        A_upper = self.An_th_th.copy()
        return A_upper
    ...
    #     upper = np.zeros_like(self.twohop_nbs) + 1e9
    #     lower = np.zeros_like(self.twohop_nbs) + 1e9

    #     for idx, i in enumerate(self.twohop_nbs):
    #         """ Eqn (6) in the paper
    #         """
    #         i_deg = self.node2deg[i]

    #         # for row upper bound
    #         u, v = self.sorted_nodes_by_deg[-2:]
    #         u_deg, v_deg = self.sorted_degrees[-2:]
    #         _upper = torch.sum(self.adj_norm[i]).item() + self._deg_norm(i_deg, u_deg+1) + self._deg_norm(i_deg, v_deg+1)
    #         upper[idx] = _upper

    #         # for row lower bound
    #         u = self.sorted_nodes_by_deg[0]
    #         v_deg = self.sorted_degrees[0]
    #         _lower = np.sum([self._deg_norm(i_deg+1, self.node2deg[v]) for v in self.adj[i].nonzero().squeeze(1).tolist()]) \
    #             + self._deg_norm(i_deg+1, v_deg+1) + 1 / (i_deg + 1)
    #         lower[idx] = _lower
    #     return lower, upper
    ...
    def _naive_entrywise_lower(self):
        A_lower = np.zeros_like(self.An_th_th)
        for i in range(len(self.twohop_nbs)):
            for j in range(len(self.twohop_nbs)):
                u, v = self.twohop_nbs[i], self.twohop_nbs[j]

                L = 1 / np.sqrt(self.degs_tilde[u] + 1) / np.sqrt(self.degs_tilde[v] + 1)
                if i == j:
                    A_lower[i,j] = L
                else:
                    A_lower[i, j] = min(self.A_th_th[i,j], L)

        # A_lower = np.tril(A_lower) + np.triu(A_lower, 1)
        # upper = np.tril(upper) + np.triu(upper, 1)        
        return A_lower
...
class BranchAndBoundCertification(GCNNodeCertification):
    ...
    def certify_fragile(self, correct_class, solver='ECOS', other_class=None):
        ...
        try:
            ...
            best_upper = best_uppers[0]
            fragile = False
            to_branch = optimization_problem
            for step in range(int(self.max_iter)):
                lower_bounds, upper_bounds = bounds(problems)
                if self.bias:
                    best_lower_bound = np.max(lower_bounds) + (c.T @ self.b2)[0]
                else:
                    best_lower_bound = np.max(lower_bounds)
                if step > 0:
                    best_lowers.append(best_lower_bound)
                ...
                if self.bias:
                    problems = purge_fragile(problems, best_lower_bound, constant=(c.T @ self.b2)[0])
                else:
                    problems = purge_fragile(problems, best_lower_bound)

                open_problems = [p for p in problems if p.open]
                if len(open_problems) == 0:
                    fragile = best_upper < 0
                    break
                
                if self.bias:
                    best_upper_n = np.max([p.upper_bound for p in open_problems]) + (c.T @ self.b2)[0]
                else:
                    best_upper_n = np.max([p.upper_bound for p in open_problems])

                best_upper = best_upper_n
                if step > 0:
                    best_uppers.append(best_upper)
                
                if best_upper < 0:
                    fragile = True
                    break

                if best_lower_bound - best_upper < self.tolerance:
                    fragile = False
                    break

                to_branch = open_problems[np.argmax([p.upper_bound for p in open_problems])]
                to_branch.branch()
            ...
            return {'fragile': fragile, 'error': False, 'best_upper': best_upper}
...
def purge_fragile(problems, best_lower, constant=0):
    new_probs = []
    for p in problems:
        if p.open and p.upper_bound + constant - best_lower < 1e-8:
            new_probs.append(p)
        if not p.open:
            new_probs.append(p)
    return new_probs
...
def certify(i, A, X, weights, z, local_changes=5, global_changes=20,
    ...
    onehop_nbs = np.array(sorted(onehop_nbs))
    twohop_nbs = np.array(sorted(twohop_nbs))
    ...
    if eval_class is None:
        ...
        for other_class in predicted_logits.argsort()[::-1]:
            ...
            # res = certification.certify_fragile(certify_class, solver=solver, other_class=other_class)
        ...
        if eval_class != certify_class:
            ...
            # res = certification.certify_fragile(certify_class, solver=solver, other_class=eval_class)
            results.update(res)
        # else:
        #     results.update({'fragile': False, 'error': False})
